...
token = ''#  <Coloque aqui o token gerado pelo BotFather>
...
def sair(message):
    ...
    try:
        ...
        if myid in markup.userStep: del markup.userStep[myid]
        if myid in markup.ofertas: del markup.ofertas[myid]
        if myid in markup.motoristas: del markup.motoristas[myid]
        if myid in markup.selection: del markup.selection[myid]
        bot.send_message(myid, txt, reply_markup=removemkp)
    except KeyError:
